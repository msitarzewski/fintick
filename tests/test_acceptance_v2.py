...
class V2AcceptanceTests(unittest.TestCase):
    ...
    def test_process_docs_are_v2_and_non_8080(self) -> None:
        services = (ROOT / "setup-fintick-services.sh").read_text()
        readme = (ROOT / "README.md").read_text()
        demo = (ROOT / "run-demo.sh").read_text()

        self.assertIn("fintick-aggregate", services)
        self.assertIn("fintick-validate", services)
        self.assertNotIn("fintick-enrich", services)
        self.assertNotIn("fintick-research", services)
        self.assertNotIn("8080", services)
        self.assertIn("8137", services)
        self.assertIn("systemctl --user", services)
