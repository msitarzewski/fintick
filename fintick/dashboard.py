...
function layoutMasonry(){const feed=$('feed');if(!feed)return;const cards=[...feed.children];if(!cards.length)return;const styles=getComputedStyle(feed),row=parseFloat(styles.gridAutoRows)||1,gap=parseFloat(styles.rowGap)||0,spacer=parseFloat(getComputedStyle(cards[0]).marginBottom)||0;try{for(const c of cards)c.style.gridRowEnd='';const heights=cards.map(c=>c.getBoundingClientRect().height);cards.forEach((c,i)=>{const h=heights[i];if(h)c.style.gridRowEnd='span '+Math.max(1,Math.ceil((h+spacer+gap)/(row+gap)))});feed.style.gridAutoRows=''}catch(error){/* Half-spanned cards overlap and are unreadable; a plain grid is not. */
for(const c of cards)c.style.gridRowEnd='';feed.style.gridAutoRows='auto'}}
